# Pathology Foundation Model Baselines for Prada Tuning
...
| Method | Pretraining and backbone | Input | Downstream adaptation | Feature used |
|---|---|---:|---|---|
| Prada Tuning | General-domain CLIP ViT-B/16  | 224 x 224 | DCP + ViSo Adapter  | Adapted image-text representation  |
| UNI + LP | Pathology-pretrained UNI ViT-L/16  | 224 x 224 | Frozen encoder + linear head  | 1,024-dimensional global representation  |
| Virchow2 + LP | Pathology-pretrained Virchow2 ViT-H/14  | 224 x 224 | Frozen encoder + linear head  | CLS token concatenated with mean patch token, 2,560 dimensions  |

## Evaluation Protocol 

The comparison uses the same four image-level classification datasets and the same shot counts as the main study. An N-shot training set contains N images per class sampled from the base training partition. For each matched run, all methods used the same cached few-shot manifest (`shot_N-seed_S.pkl`). Validation uses at most four images per class, and final accuracy is measured on the complete base test partition.
...
| Item  | Setting  |
|---|---|
| Datasets  | BACH, BreakHis, MHIST, and PatchCamelyon (PCam) |
| Prediction unit  | Individual image  |
| Few-shot settings  | 1, 2, 4, 8, and 16 labeled training images per class  |
| Validation sampling  | At most four images per class |
| Test set  | Complete base test partition for every run  |
| Primary metric  | Top-1 classification accuracy (%)  |
| Summary statistic  | Mean +/- standard deviation across matched seeds  |
...
| Item  | Setting  |
|---|---|
| Trainable component  | Linear head weights and bias only  |
| Loss  | Cross-entropy  |
| Optimizer  | SGD |
| Initial learning rate  | 0.0035 |
| Training epochs  | Maximum epochs are set to 200 for 16/8 shots, 100 for 4/2 shots, and 50 for 1 shot, following CoOp. |
| Training batch size  | 4 |
| Learning-rate schedule  | Cosine decay  |
| Warm-up  | 1 epoch, constant warm-up learning rate of 1e-5  |
| Training transforms  | Random resized crop, random horizontal flip, normalization  |
| Encoder precision  | Automatic mixed precision  |
| Classifier precision  | FP32 |
| Model selection  | Final training epoch (`last_step`); no early stopping  |
...
<!-- **中文：** 此外，本次评估仅限于四个公开的图像级分类数据集。由于缺少必要的分组元数据，BACH 和 MHIST 的患者级或病例级独立性无法得到验证。BreakHis 的各数据划分在图像层面互不重叠，但并非患者级互斥。因此，所报告的结果不应被解释为患者独立泛化力、临床实用性或全切片图像任务优越性的证据。-->
